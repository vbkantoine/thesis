...
Finally, in response to \textbf{question iii}, we address the challenge of solving the optimization problem that defines references priors. % in general.
% computational cost associated with reference prior implementation. 
...
Indeed, the proposed methodology %aims to provide answers to the six main questions identified in this research work 
handles many of the central issues identified throughout this research,
and which concern the objectivity and robustness of the estimates, the ease of implementation and the achievement of minimal estimation bias. By an appropriate choice of data, our methodology makes the estimates robust. It allows to very effectively reduce the probability of likelihood degeneration for a given sample size. In the framework that we propose, we also found that, in doing so, the \emph{a posteriori} estimation is very little influenced by the choice of the constraint that aims to make the prior proper. In other words, the methodology tends to reduce the ``influence'' of the prior on the estimations of the fragility curves. %, although this is not theoretically proven. 
%However, this is the meaning of the criterion that was developed for the design of experiments because its objective is to sequentially choose the data that have a maximum impact on the posterior. 
