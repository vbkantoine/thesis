...
In this chapter, we conduct a Bayesian estimation of seismic fragility curves. Seismic fragility curves, which are defined as the probability of failure of a mechanical equipment conditional to an intensity measure (IM) derived from the seismic scenario, are reviewed in \cref{chap:frags-intro}. We still address case studies for which (i) the information about the structure's response is limited to binary outcome (i.e., failure or non-failure) and (ii) the available data are scarce. 
...
Indeed, the information \emph{a priori} coexists with the information that comes from the observations, and that is incorporated through the statistical model.
...
This thought arises as a cornerstone when it resorts to estimate seismic fragility curves.
Indeed, the developments done in \cref{chap:prem} and \cref{chap:constrained-frags}
showed that (i)~the information \emph{a priori} must be carefully defined, since any embedded subjectivity in its design may significantly impact the estimates, and (ii)~the information that comes from the data may take ``degenerate'' forms, jeopardizing efficiency.
%it embedds
...
In these chapters, we proved that the prior can be designed in a way that 
permit accurate estimations of seismic fragility curves, without being affected by degenerate scenario.
%proved how t
% We proved that the first one can be designed 
Nevertheless, the information that comes from the data still has a prominent influence on the estimates. %The more the data are close to a degeneracy, the %more 
%less the posterior
As a matter of fact, two datasets of the same size do not contain the same amount of knowledge, and one will inform more the posterior than the other. 
Optimizing that information inevitably guide the result away from the prior, enhancing objectivity as well.
...
That paradigm is not only used to construct the prior, ensuring its objectivity, but also to define a criterion from which is based our experimental design.
...
This means  the latter has its own influence on the estimates. In our framework,
...
\section{Estimates and benchmarking metrics}




\section{Application of the method on the piping system}\label{sec:application}
...
\section{Behavior of the method on toy case studies}\label{app:toycases}
