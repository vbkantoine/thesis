...
%
    Considering two different IMs (PGA vs. PSA), we remark that a more correlated IM to the structural response is more likely to yield degenerate scenarios. 
    Such degeneracy compromises fragility curves estimations, either with Bayesian or classical frequentist methods.
    The consequences of this remark on the estimates are thoroughly investigated on a case study, and compared for different approaches including ours.
    ...
    We precise that the case study considered here is not the same as the piping system presented in \cref{chap:frags-intro} and studied in some other chapters. In this work, a linear behavior of the structure is assumed to increase the correlation between its response and the IM, in order to highlight phenomena of interest.
\end{abstract}
