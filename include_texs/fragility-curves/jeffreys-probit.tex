...
\begin{abstract}
    ...
    and among various methods that exist to estimate the parameters of the model, the Bayesian inference became popular in the literature. However, the latter's trustworthiness relies on the prior selection, jeopardizing the approach if the selection is motivated by subjective consideration.
    This chapter proposes a comprehensive Bayesian estimation of seismic fragility curves, implementing the Jeffreys prior for the probit-lognormal model.
    ...
    It is also implemented on three different case studies, including an industrial example. %--- to illustrate the theoretical 
...
That statement paves the way for further work that could (i)~reinforce the prior construction to suppress its posteriors' weaknesses under degenerate likelihoods; and (ii)~study the probability and the methods to reduce that of the existence of degenerate likelihoods. % an and the methods to reduce 
