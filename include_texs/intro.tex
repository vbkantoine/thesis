...
\begin{abstract}
    In this chapter, we introduce and motivate the research work carried out through this thesis. The work is motivated
    mainly by the need to evolve seismic probabilistic risk assessment study methods, but also by the lack of response provided by the state of the art to the question of the choice of prior in Bayesian inference.
       These issues are detailed, and
    the work is then introduced from their analyses, resulting in a consistent organization of the manuscript.
\end{abstract}
...
The incident that occurred in 2011 at the Fukushima-Daiichi nuclear power plant illustrates this clearly. The seismic hazard (which triggered the tsunami) had been underestimated by the consensus of Japanese experts, and it is ultimately this underestimation that led to the accident (see the report by the international atomic energy agency, \cite{iaea_fukushima_2015}). In the wake of this event,
 the ASNR decided to amplify the ``augmented safety earthquake'' by a factor of 1.5 in France, requiring operators to provide an amplified demonstration of the robustness of their installations.
...
Each of these publications, whether it takes the form of a journal paper or a proceedings paper propose an original contribution that is tied to this thesis.
...
The first exception is the proceedings paper published at JdS in 2023. Indeed, this work constituted an introduction to the study which led to the journal paper entitled ``Generalized mutual information and their reference priors under Csizar f-divergence''. In this manuscript, the proceedings paper is incorporated to the chapter tied with the latter (\cref{chap:ref-generalized}). The second exception is the proceedings paper published at SMiRT in 2025. The original methodology presented in this paper remains conceptual and its development faced the end of  the thesis.
The study thus lacks for an in-depht-look that might be conducted in the future.
...
L'ASNR se place alors en arbitre dans ce dialogue, entre autre elle définit le ``séisme majoré de sécurité''\footnote{Aujourd'hui plutôt appelé ``séisme noyau dur''.} (il s'agit d'une majoration du spectre du ``séisme maximal historiquement vraisemblable''\footnote{Aujourd'hui plutôt appelé ``séisme de dimensionnement''.}) qui produit la marge de référence dans la démonstration de robustesse parasismique d'un équipement. % la marge de fiablité et séisme maximal à prendre en compte. %En ce qui concerne l'aléa sismique, le dialogue 
%
