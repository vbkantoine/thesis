...
L'identification des incertitudes sur l'entrée $\mbf X$ ou sur le modèle même $\cM$ (ou bien le méta-modèle) peut se faire au travers d'une paramétrisation de celles-ci, en donnant lieu à une distribution paramétrique de $\mbf Y$ : $\PP_{\mbf Y|\mbf X,\theta}$. La variable $\theta$, aléatoire sous le paradigme bayésien, peut alors représenter diverses sources d'incertitudes, de différentes natures.
...
\begin{itemize}
    \item[\textbf{Question 1}] how to define objective priors ?
    \item[\textbf{Question 2}] if they are non informative how to workaround their issues ?
    \item[\textbf{Question 3}] how to construct them in practice ?
    \item[\textbf{Question 4}] what does they look like in the context of the spra ?
    \item[\textbf{Question 5}] what are their limit in this context and how to enhence them ?
    \item[\textbf{Question 6}] how can we leverage the best the objective priors in the bayesian  information worflow to provide the best possible estimation of seismic fragility curves ?
\end{itemize}
