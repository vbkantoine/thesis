...
\begin{abstract}[\hspace*{-10pt}]
    This chapter is mainly based on the submitted work: \fullcite{van_biesbroeck_properly_2024}  % Ce chapitre reprend principalement les travaux publiés dans: 
\end{abstract}
...
\begin{defi}[Generalized reference prior]\label{def:BA:genref}
    ...
    % with $\pi^\ast(\Theta_i)>0$, $\Theta_i\subset\Theta$, $\bigcup_{i\in I}\Theta_i=\Theta$ such that
        ...
        \end{equation}
    where  $\varphi(k)$ is a {positive and}  monotonous function of $k$. It is said to be unique if for any other $D$-reference prior $\varPi'$, $\varPi\simeq\varPi'$.
\end{defi}
...
\begin{rem}\label{rem:klconst}
    ...
    In \cref{app:chap:nonasympt} we prove that the expression using the Kullback-Leibler divergence would take the form:
