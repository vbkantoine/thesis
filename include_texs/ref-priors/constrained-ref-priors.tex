...
\begin{thm}\label{thm:lintoproper}
    ...
    $\varPi\in\overline\cP$ as the prior whose density $\pi$ verifies % follows 
        ...
        \end{equation}
    If $Jg$ is non-integrable in the neighborhood of $b$, then $\varPi$ is a $D_\delta$-reference prior over $\overline\cP$. Otherwise, and if $J$ is improper in the neighborhood of $b$, $\varPi$ is a $D_\delta$-reference prior over the class of proper priors in $\overline\cP$.
\end{thm}
...
where $f$ is a density function with support on $\RR$, assumed to be symmetric with a single mode at zero, and with a few integrability assumptions that are detailed in \cite{rubio_inference_2014}.
    The choice of $f$ is open and we can take, % let the assumptions of Theorem \ref{thm:l(pi)} to be verified.
    %\textcolor{red}{
    %We may take, 
    ...
    \end{equation}
    it is improper and leads to an improper posterior. In the following, we construct different priors based on the suggestions developed in this paper. %a method which, on the basis of our suggestions developed in what precedes, to construct a more suitable prior.

    \paragraph{Proper priors based on a moment constraint}
    Considering results in Section \ref{sec:constrainedproper} and the decay rates of the Jeffreys prior above, a simple correction can be done to issue a proper reference prior w.r.t. $\sigma_1,\sigma_2$, which results in a proper posterior.
...
Our prior densities are compared with the Jeffreys prior one on Figure \ref{fig:priorpost}.(a), w.r.t. the parameter $\sigma_2$ the others being fixed to $1$. For this comparison, a multiplicative constant had to be chosen on $J$, we have chosen the one such that $J(1,1,1)=2$.
...
Finally, note that in Figure \ref{fig:priorpost}.(a) is also drawn the \emph{independent Jeffreys prior} proposed by the authors in \cite{rubio_inference_2014} for this model as an alternative to Jeffreys. Its decay rates are actually the same as the ones of $\pi_\gamma$ when $\gamma=1/2$, and the two priors are hard to distinguish. Note that this latter prior $\pi_{1/2}$ equals the hierarchical reference prior (as described in Section \ref{sec:issuesJeff}) constructed from the ordering $\pi(\theta)=\pi_1(\sigma_1,\sigma_2|\mu)\pi_2(\mu)$.
...
In Figure \ref{fig:priorpost}.(b), (c) and (d) are plotted, for numerous $\gamma$ and several data set sizes $k$, the posterior densities that result from a sample of data and from the prior densities $\pi_\gamma$.
...
For practical details, the chosen $f$ is the standard Gaussian density, and the different data sets have been generated according to the likelihood in Equation (\ref{eq:examplelikilihood}) conditionally to $\theta^\ast=(2,2,2)$. %They were of size $k=50$, and a number $N=1000$ of them have been generated for the computation of the median densities.
...
%    In accordance with the criticize of \citet{Berger2009} about the derivation of the Jeffreys prior on the full parameter space aforementioned in Section \ref{sec:issuesJeff}, a hierarchical construction of the Jeffreys prior is to be favored. From the Fisher information matrix in Equation (\ref{eq:fishermatrix}) we obtain:
%        \begin{equation}
...
\begin{figure}[h]%
    ...
    \caption{On (a), different prior densities $\pi_\gamma$ (in dashed line) w.r.t. $\sigma_2$ along with a Jeffreys prior density (that delimits the blue area) and the \emph{Independent Jeffreys} of \citep{rubio_inference_2014} (that delimits the orange area). On each of (b), (c) and (d), posterior densities for different values of $\gamma$ are plotted. %One observed sample served the calculation of all the posteriors on one figure. 
...
\subsection{Proof of Theorem \ref{thm:Jthetaa}}
...
Let us define the increasing sequence of compact subsets $\Theta$: $\Theta_i=[\theta_i,1]$, $i\geq0$, with $\theta_i\conv{i\rightarrow\infty}0$. We denote by $\psi_i$ and $\tilde\psi_i$ the functions defined as follow:
    ...
    \end{equation}
The quantity $\psi_i(u)$ corresponds ---up to a positive constant--- to the parametrization w.r.t. $u\in\RR$ of $l(\pi_i)$; where $\pi(\theta)\propto\theta^u$, $\pi_i$ is re-normalized restriction of $\pi$ to $\Theta_i$  and where $l$ is the function defined in Theorem \ref{thm:l(pi)} that we seek to maximize.
...
\paragraph{The case $\mathbf{a<-1}$}
    ...
    Let us now work beyond the subset $U$ of $\RR$. First, if $u\in(-1,+\infty)$, the integrals that compose $\psi_i(u)$ both admit finite and positive limits when $i\to\infty$. The limit of $(|\psi_i(u)|)_i$ is moreover bounded from below as a consequence of Equation (\ref{eq:gendarmeJeffreys}):
    ...
    As a result, for any $i>i_2$:
        ...
        \end{equation}
    Finally, if $u\in(u_m,-1)$, analogously than in Equation (\ref{eq:minorationpsiu1infty}), we can write
        \begin{equation}
...
\subsection{Proof of Proposition \ref{prop:constraints}}
...
%$\pi^\ast$ over a convex class such as $\tilde\cP$ must maximize $l$ as a prior on $\Theta$. The mapping $\pi\mapsto l(\pi)$ being strictly convex when $\pi$ is seen as a function in $\cR^\cC$, such maximal argument is unique.
...
% \paragraph{Proof of Lemma \ref{lem:constrainstaec0}}
...
Therefore, calling $\tilde\cP^\ast$ the class of positive priors in $\tilde\cP$, we have $\argmax_{M(\pi)\in\tilde\cP}l(\pi)=\argmax_{M(\pi)\in\tilde\cP^\ast}l(\pi)$ which is $\pi^\ast$.
...
% Clearly, $\pi^\ast$ as given by the lemma in Equation (\ref{eq:lemconstepiast}) is a prior in $\tilde\cP\cap U\subset U\cap C$ because the functions $g_j$ are continuous as well as Jeffreys is. Additionally, $\tilde\cP$ differs from $\tilde\cP\cap U$ as it admits non-negative priors on top of positive ones. Actually, $\tilde\cP$ is the closure of $\tilde\cP\cap U$ for the topology induced by the infinite norm $\|\cdot\|$ ($\|f\|=\sup_\Theta|f|$), for which $l$ is continuous. Eventually, $\argmax_{\pi\in\tilde\cP}l(\pi)=\argmax_{\pi\in\tilde\cP\cap U}l(\pi)=\argmax_{\pi\in U\cap C}l(\pi)$, hence the result.



\subsection{Proof of Theorem \ref{thm:lintoproper}}
...
Thus, $\pi^\ast_i$ is a maximal argument of $l$ under the constraints 
    ...
    \end{equation}
Given the result of Proposition \ref{prop:constraints}, $\pi^\ast_i$ takes the form of:
    \begin{equation}
...
\subsection{Proof of Theorem \ref{thm:quasipostpropre}}
  ...
  must maximize the function $l$ under the constraints $\int_{\Theta_i}\pi_i=1$ and $\int_{\Theta_i}\pi_i g=c_i$. Thus, according to Proposition \ref{prop:constraints}, $\pi_i$ can be written as:
     \begin{equation}
...
This way, Equation (\ref{eq:convexityConspi1}) does not allow $\li2$ to be non-negative.
...
%\end{equation}
$\li2 \aseq{i\rightarrow\infty} o\left(\li1\left(\int_{\Theta_i}Jg\right)^\delta\right) $
and the right hand side in Equation (\ref{eq:convixity2l2leq0}) diverges to $\infty$, which is a contradiction with $(c_i)_i$ being bounded.
