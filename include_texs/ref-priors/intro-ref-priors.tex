...
Given the observations $\mbf y$, the \textbf{posterior distribution}, denoted $\PP_{T|\mbf y}$, is then defined as the a.s. unique one verifying
    \begin{equation}
...
\begin{defi}[Reference priors]\label{def:intro-ref:ref-priors}
    Let $\cP$ be set of priors on $\Theta$, a prior $\varPi\in\cP$ %whose prior density is $\pi$ 
...
The posterior distribution $\PP_{T|\mbf y}$ is well-defined for almost every $\mbf y\in\cY^k$ by
\begin{equation}
    \forall A\in\sY^{\otimes k},\,\forall B\in\sT,\, \int_{A}\PP_{T|\mbf y}(B)d\PP_{\mbf Y_k}(\mbf y) = \int_B \PP_{\mbf Y_k|\theta}(A)d\varPi(\theta).
\end{equation}
...
\begin{defi}[Advanced defintinon of reference priors]
    Let $\cP$ be a set of priors. A prior $\varPi\in\cP$ is a reference prior over $\cP$ if there exist an openly increasing sequence of compacts $(\Theta_i)_{i\in\NN}$ such that $\bigcup_{i\in\NN}\Theta_i=\Theta$ with for any $i$: $0<\varPi(\Theta)<\infty$ and
    \begin{equation}
...
is stated below. It is, according to ourselves, an appropriate condition on this sequence, as it allows the consistency between the compact case and the improper case.
...
That let them be elected for their lack of subjectivity in various practical
studies (e.g. \cite{Chen2008,Gu2016,Dandrea2021}) 
and various statistical models, such as Gaussian process-based models \citep{paulo_default_2005,gu_parallel_2016}, generalized linear models \citep{natarajan_reference_2000} or genearal exponential family models 
...
to be qualified as objective


referen ce priors are designed to be  objective to do so they

the theory is well developed the priors are studied the come with tremendious properties and even solutions in some cases.*

and the framework that we introduce allows a convincing carrying out of improper priors

However some limit remains. anyway th objectivity coms from a a chosen expression of what they should maximize
and their defintion remaon not fully stated in the mutlidimensional
Indeedn we avoid Jeffreys for several of its defaults such as.
