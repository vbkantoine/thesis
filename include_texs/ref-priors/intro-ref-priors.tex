...
This chapter proposes a novel brief review of the reference prior theory, fixing the framework and defining its main tools (in \cref{sec:intro-ref:classical-framework}). %, such as the mutual information. 
...
\begin{equation}
    \forall A\in\sY^{\otimes k},\, \PP_{\mbf Y}(A) = \int_\Theta \PP_{\mbf Y|\theta}(A)d\varPi(\theta),
\end{equation}
...
Given the observations $\mbf y$, the \textbf{posterior distribution}, denoted $P_{T|\mbf y}$, is then defined as the a.s. unique one verifying
    \begin{equation}
        \forall A\in\sY^{\otimes k},\,\forall B\in\sT,\, \int_{A}\PP_{T|\mbf y}(B)d\PP_{\mbf Y}(\mbf y) = \int_B \PP_{\mbf Y|\theta}(A)d\varPi(\theta).
    \end{equation}
...
This way, $\PP_{\mbf Y}$ admits a \textbf{marginal density} $p_{\mbf Y}$ w.r.t. $\mu^{\otimes k}$ defined as
    \begin{equation}
        \forall\mbf y\in\cY^k,\, p_{\mbf Y}(\mbf y) = \int_\Theta\prod_{i=1}^k\ell(y_i|\theta) d\varPi(\theta) = \int_\Theta \ell_k(\mbf y|\theta)d\varPi(\theta),
    \end{equation}
...
This construction is called a way-around because the framework remains incomplete: $T$ is not appropriately defined anymore, nor the marginal distribution $\PP_{\mbf Y}$. Later in this chapter, we will introduce a novel construction that authorizes the definition of improper prior in a more suitable way.
...
\begin{defi}[Mutual information]\label{def:intro-ref-MI}
    ...
    \begin{equation}
        \sI^k(\varPi) =  \EE_{\mbf Y\sim \PP_{\mbf Y}}[\text{KL}(\PP_{T|\mbf Y}||\varPi)] =  \int_{\cY^k}\text{KL}(\PP_{T|\mbf y}||\varPi) p_{\mbf Y}(\mbf y)d\mbf y.
    \end{equation}
\end{defi}
...
\begin{equation}
    \sI^k(\varPi) =  \EE_{\mbf Y\sim \PP_{\mbf Y}}[\text{KL}(\PP_{T|\mbf Y}||\varPi)] =  \EE_{T\sim\varPi}[\text{KL}(\PP_{\mbf Y|\theta}||\PP_{\mbf Y}) ]
\end{equation}
...
\begin{defi}[Reference priors]\label{def:intro-ref:ref-priors}
    Let $\cP$ be class of priors on $\Theta$, a prior $\varPi\in\cP$ %whose prior density is $\pi$ 
    is a reference prior over $\cP$ if there exist an appropriate increasing sequence $(\Theta_i)_{i\in\NN}$ such that $\bigcup_{i\in\NN}\Theta_i=\Theta$ with for any $i$: $0<\varPi(\Theta_i)<\infty  $ and
        ...
        \end{equation}
    with equality if and only if $\varPi_i=P$; with $\varPi_i$ (resp. $P_i$) being the restriction of $\varPi$ (resp. $P$) on $\Theta_i$, $\sI_i^k$ denoting the mutual information on the restricted modeling driven by $\Theta_i$, and $\cP_i=\{P\in\cP,\,0<P(\Theta_i)<\infty\}$.
\end{defi}

%This definition differs slightly from the one in \cite{berger_formal_2009}
%This definition will serve as a reference. I
This first definition considers a set $\cP$ called a class of priors. 
...
\end{equation}
We can define $\varPi$ by
    \begin{equation}
        \varPi\Big(N\times\prod_{i\in I}E_i\Big) = \sum_{n\in N}\PP^n(E_n)\pi(U_n),
    \end{equation}
for any $N\in\sP(I)$, $(E_i)_{i \in I} \in\prod_{i\in I}\sX^i$ with $E_i=\Omega^i$ for any $i\geq k$ for some $k<\mbf i$. 
The existence of the measure $\mbf\varPi$ on $\mbf\Xi$, that coincides with $\varPi$ on the cylinder sets is guaranteed by  Carathéodory's extension Theorem. %$^\ast$
...
for any $V\in\sT$:
    \begin{align}
          \mbf\varPi(T\in V) &= \varPi\Big(\bigcup_{i\in I}\{i\}\times\Omega^1\times\dots\times\{T^i\in V\}\times\Omega^{i+1}\times\dots\Big)\\
            %&
          %I\times\prod_{i\in I} \{T^i\in V\}\Big) = 
          &=\sum_{i\in I}\PP^i(T^i\in V)\pi(U_i) = \sum_{i\in I}\pi(V|U_i)\pi(U_i) = \pi(V);\nonumber
    \end{align}
and if $\pi(V)<\infty$ then for any $(A_{j_l})_{l=1}^k\in\sY^k$: %, with the notation $\{Y_{i_l}\in A_{i_l}\}=\{Y_{i_l}(p)\in A_{i_l}\}$ to see that event as en event in $\mbf\Omega$:
    \begin{multline}
        \mbf\varPi\left((Y_{j_l})_{l=1}^k\in(A_{j_l})_{l=1}^k|T\in V\right)\mbf\varPi(T\in V) \\ = 
        \mbf\varPi\Big(\bigcup_{i\in I}  \{i\}\times\Omega^1\times\dots\{(Y^i_{jl})_{l=1}^k\in(A_{jl})_{l=1}^k\}\cap \{T^i\in V \}\times\Omega^{i+1}\dots 
        \Big),
    \end{multline}
    so that
        \begin{align}\nonumber
        &\mbf\varPi\left((Y_{j_l})_{l=1}^k\in(A_{j_l})_{l=1}^k|T\in V\right) 
...
Also $\mbf\varPi(I^n\times\prod_{i\in I}E_{i,0}')=\sum_{i=0}^{n-1}\pi(U_i)<\infty$.
Therefore, we can write:
    \begin{align}
        \mbf\varPi\Big(\bigcap_{k<\mbf i}(I^n\times\prod_{i\in I}E_{i,k}')\Big) 
            &= \lim_{k\rightarrow\infty} \mbf\varPi\Big(I^n\times\prod_{i<k,\mbf i}E_i\times\prod_{k\leq i<\mbf i}\Omega^i\Big) \\
